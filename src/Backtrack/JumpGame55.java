...
public class JumpGame55 {
    public static void main(String[] args) {
        int[] nums = {1, 1, 2, 2, 0, 1, 1}; //3,2,1,0,4 //1,1,2,2,0,1,1
        System.out.println(canJump(nums)); //simpler, faster
        System.out.println(canJump2(nums)); //shows dp + memo, slower
    }
    ...
    public static boolean canJump2(int[] nums) {
        List<Boolean> memo = new ArrayList<>();
        for (int i = 0; i < nums.length; i++) {
            memo.add(false);
        }
    ...
    public static boolean jump(int[] nums, int index, List<Boolean> memo) {
        if (memo.get(index)) {
            return false;
        }
        memo.set(index, true);
        if (index == nums.length - 1 || nums[index] + index >= nums.length) {
            return true;
        }
        if (nums[index] == 0) {
            return false;
    ...
    public static class MinimumAsciiDeleteSum712 {
        public static void main(String[] args) {
            String s1 = "delete";
            String s2 = "leet";
